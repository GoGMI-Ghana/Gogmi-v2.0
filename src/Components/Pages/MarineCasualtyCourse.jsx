...
// Server path: public_html/api/api/courses/marine-casualty.php
...
const MarineCasualtyCourse = () => {
  ...
  // ── Verify Membership ID against backend ───────────────────────────────────
    ...
    try {
      ...
      if (data.success) {
        ...
        setMemberForm({
          ...
          institution: data.data.organization || '',
          country:     data.data.country     || ''
        });
        setApplyStep('member');
      } else {
        setMemberCodeError(data.message || 'Invalid Membership ID');
      }
    } catch {
      setMemberCodeError('Unable to verify. Please check your connection.');
    } finally {
  ...
  const processPayment = (email, amountUSD, applicantType, formPayload) => {
    ...
    try {
      const handler = window.PaystackPop.setup({
        ...
        callback: (response) => {
          activateRegistration(response.reference, applicantType, formPayload);
        },
        onClose: () => {
  ...
  // ── Call backend to activate registration after payment ────────────────────
    ...
    try {
      const res  = await fetch(API_URL + '/courses/marine-casualty.php', {
        ...
        body:    JSON.stringify({
          ...
          applicantType,
          paymentReference,
          membershipId: applicantType === 'member' ? memberCode.trim() : undefined
        })
      });
      const data = await res.json();
      ...
      if (data.success) {
        let msg = 'Registration Successful!\n\n'
                + 'You are now registered for the Marine Casualty Investigation & Safety Management Course.\n'
                + 'Reference: ' + paymentReference + '\n';

        if (data.data.autoMembership) {
          msg += '\nCongratulations! You are now a GoGMI Associate Member.\n'
        ...
        alert(
          ...
          '\nReference: ' + paymentReference +
          '\nPlease contact info@gogmi.org.gh'
        );
      }
    } catch {
      alert(
        'Registration error. Please contact info@gogmi.org.gh\n'
        + 'Reference: ' + paymentReference
      );
    } finally {
      setIsProcessing(false);
  ...
  const handleMemberSubmit = (e) => {
    e.preventDefault();
    if (!memberForm.fullName || !memberForm.email || !memberForm.phone || !memberForm.country || !memberForm.position) {
  ...
  const handleNonMemberSubmit = (e) => {
    e.preventDefault();
    if (!nonMemberForm.fullName || !nonMemberForm.email || !nonMemberForm.phone || !nonMemberForm.country || !nonMemberForm.position) {
      alert('Please fill in all required fields');
      return;
    }
  ...
  const handleBrochureDownload = () => {
    ...
    l.href     = '/resources/pdfs/Marine-Casualty-Course-Brochure.pdf';
    l.download = 'Marine-Casualty-Course-Brochure.pdf';
    l.target   = '_blank';
  ...
  // ── Static data ────────────────────────────────────────────────────────────
  const modules = [
    { number: 1, title: "Marine Casualty Investigation – Concepts, Scope and Legal Framework", icon: <Shield className="w-6 h-6" />, color: "#8E3400", topics: ["Understanding the importance of marine casualty investigations","Understanding the nature and types of marine casualties","Legal definition and classification under national and regional provisions (Ghana Shipping Act)","IMO Casualty Investigation Code and related SOLAS requirements","Detailed study of Sections of the IMO Casualty Investigation Code","Roles of the Maritime Administration, Wreck Commissioner, and Assessors","Procedures for preliminary inquiry and formal investigation","The 'Stop Rule'","Appeals, rehearing, and disciplinary actions on certificates","Coordination between national agencies (law enforcement agencies)"], learningOutcomes: ["Explain the philosophy, importance and international background of marine casualty investigations","Identify key international instruments (IMO Casualty Investigation Code, SOLAS requirements)","Describe the roles, powers and responsibilities of Maritime Administration, Wreck Commissioner and Assessors","Outline the full procedural sequence for preliminary inquiries, formal investigations, appeals","Assess inter-agency coordination responsibilities in casualty response"] },
    { number: 2, title: "Investigation Procedures and Evidence Handling", icon: <Search className="w-6 h-6" />, color: "#132552", topics: ["Step-by-step process of investigation","Scene management and preservation of evidence","Witness interviewing and record-keeping","Application of the causal chain and root cause analysis models","The 5 Whys technique for drilling down to root causes","Fishbone (Ishikawa) diagram for categorizing contributing factors","Barrier analysis: identifying failed or missing controls","Drafting structured investigation reports"], learningOutcomes: ["Execute a systematic step-by-step investigation process from incident notification to completion","Apply scene management techniques to preserve physical and testimonial evidence","Conduct effective witness interviews and maintain accurate investigation records","Use causal chain and root cause analysis models to identify underlying causes","Produce structured investigation reports with clear findings and recommendations"] },
    { number: 3, title: "Safety Data Management and Reporting", icon: <BarChart3 className="w-6 h-6" />, color: "#8E3400", topics: ["Standard formats for casualty reporting and documentation","Integration of data into national and IMO systems (GISIS)","Overview of Global Integrated Shipping Information System (GISIS)","Types and sources of maritime safety data","Data recorders: Voyage Data Recorder (VDR), Simplified Voyage Data Recorder (SVDR)","Safety trend analysis and data visualisation techniques","Risk matrices and ALARP (As Low as Reasonably Practicable) presentations","Developing feedback loops for policy improvement"], learningOutcomes: ["Prepare casualty reports using standard formats and terminology","Integrate investigation data into national and IMO reporting systems including GISIS","Analyse safety trends using data visualization techniques and risk assessment tools","Develop effective feedback mechanisms to inform policy and operational improvements"] },
    { number: 4, title: "Human Factors, Safety Culture, and Crisis Response", icon: <Brain className="w-6 h-6" />, color: "#132552", topics: ["Human performance, organisational culture, and accident causation","Stress and its impact on performance","Fatigue and vigilance degradation","Drug and alcohol impairment","Communication breakdowns and failure modes","Organisational and management (O&M) factors","Decision-making under pressure and coordination failures","Communication and leadership during emergency response","Media management during crisis","Promoting proactive safety behaviour across agencies and coastal communities"], learningOutcomes: ["Analyse human performance factors and organizational culture elements that contribute to marine accidents","Evaluate decision-making processes under pressure and identify common coordination failures","Apply effective communication and leadership principles during emergency response","Develop strategies to promote proactive safety behaviour across agencies and coastal communities"] },
    { number: 5, title: "Basic Analysis of Marine Casualties", icon: <Microscope className="w-6 h-6" />, color: "#8E3400", topics: ["Understanding the purpose and principles of casualty analysis","Applying critical thinking to marine accident events and circumstances","Distinguishing between immediate, underlying and root causes","Identifying human, technical, organisational and environmental contributing factors","Using basic analytical techniques to examine evidence and sequence of events","Human Factors Analysis and Classification System","Differentiating facts, assumptions and inferences in casualty analysis","Producing logical summaries of analytical findings"], learningOutcomes: ["Apply structured critical thinking principles when examining marine casualty events","Identify and categorise immediate causes, underlying causes and root causes","Use simple analytical tools and methods to evaluate events and contributing factors","Distinguish between facts, assumptions, inferences and opinions","Prepare clear, logical and concise analytical summaries"] },
    { number: 6, title: "Case Study - Mock Marine Accident Investigation and Report", icon: <FileText className="w-6 h-6" />, color: "#132552", topics: ["Comprehensive analysis of provided accident scenario","Evidence evaluation and documentation","Root cause identification and analysis","Sequence of events visualization","Development of findings and recommendations","Formal investigation report preparation","Practical application of all course learnings"], learningOutcomes: ["Integrate learning from across the course to a topical case study situation","Produce a detailed and well-structured investigation report","Apply all investigation techniques to a realistic scenario"] }
  ];

  const targetParticipants = ["Maritime Law Enforcement Agencies","Maritime Administrations","National and Regional Transport Authorities","Marine Surveyors","Maritime Lawyers, Prosecutors, and Regulatory Professionals","Vessel Operators and Owners, and Safety Managers","Representatives from Academia and Civil Society working in Maritime Safety and Governance"];
  ...
  const courseHighlights    = [{ icon: <Shield className="w-5 h-5" />, title: "IMO Compliance", description: "Aligned with IMO Casualty Investigation Code (MSC.255(84)) and SOLAS Chapter XI-1 Regulation 6" },{ icon: <Users className="w-5 h-5" />, title: "Expert Faculty", description: "Maritime safety experts, IMO certified investigators, and regional specialists" },{ icon: <Globe className="w-5 h-5" />, title: "Hybrid Format", description: "In-person for Ghana participants, virtual for international attendees" },{ icon: <Award className="w-5 h-5" />, title: "Certification", description: "Certificate upon successful completion of all modules and case study" }];
    ...
    <div className="w-full bg-white" style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.8)' }}>
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="sticky top-0 bg-white border-b p-6 flex items-center justify-between z-10" style={{ borderColor: '#E5E7EB' }}>
              ...
              <button onClick={closeApply} disabled={isProcessing} className="p-2 rounded-full hover:bg-gray-100 disabled:opacity-40">
            ...
            {applyStep === 'choose' && (
              <div className="p-8">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                  <button onClick={() => setApplyStep('verify')} className="group p-6 rounded-2xl border-2 text-left transition-all hover:shadow-xl hover:scale-105" style={{ borderColor: '#132552' }}>
                    ...
                    <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold text-white mb-3" style={{ backgroundColor: '#16A34A' }}>
                      <Tag className="w-3 h-3" />Save ${DISCOUNT} ({DISCOUNT_PERCENT}% off)
                    </div>
                ...
                <p className="text-center text-sm" style={{ color: '#9CA3AF' }}>
                  Not yet a member?{' '}
                  <a href="/membership" className="font-bold hover:underline" style={{ color: '#8E3400' }}>Join GoGMI</a>
                  {' '}to unlock member pricing.
                </p>
              </div>
            )}

            {/* ── STEP 2: Verify Membership ID ── */}
            {applyStep === 'verify' && (
              <div className="p-8">
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    placeholder="e.g. GoGMI-MAST2026-00001"
                  />
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={verifyMemberCode} disabled={isVerifying} className="flex-1 px-6 py-3 rounded-lg font-bold text-white transition-all hover:opacity-90 disabled:opacity-50" style={{ backgroundColor: '#132552' }}>
            ...
            {applyStep === 'member' && (
              <form onSubmit={handleMemberSubmit} className="p-8">
                ...
                <div className="space-y-4">
                  <div><label className={labelClass} style={{ color: '#132552' }}>Full Name <span className="text-red-500">*</span></label><input type="text" name="fullName" value={memberForm.fullName} onChange={handleMemberChange} required className={inputClass} style={{ borderColor: '#E5E7EB' }} placeholder="John Doe" /></div>
                  <div><label className={labelClass} style={{ color: '#132552' }}>Email Address <span className="text-red-500">*</span></label><input type="email" name="email" value={memberForm.email} onChange={handleMemberChange} required className={inputClass} style={{ borderColor: '#E5E7EB' }} placeholder="john@example.com" /></div>
                  ...
                  <div><label className={labelClass} style={{ color: '#132552' }}>Position/Title <span className="text-red-500">*</span></label><input type="text" name="position" value={memberForm.position} onChange={handleMemberChange} required className={inputClass} style={{ borderColor: '#E5E7EB' }} placeholder="Marine Safety Officer" /></div>
                  <div><label className={labelClass} style={{ color: '#132552' }}>Institution/Organisation <span className="text-red-500">*</span></label><input type="text" name="institution" value={memberForm.institution} onChange={handleMemberChange} required className={inputClass} style={{ borderColor: '#E5E7EB' }} placeholder="e.g. Ghana Maritime Authority" /></div>
                  <div><label className={labelClass} style={{ color: '#132552' }}>Country <span className="text-red-500">*</span></label><input type="text" name="country" value={memberForm.country} onChange={handleMemberChange} required className={inputClass} style={{ borderColor: '#E5E7EB' }} placeholder="Ghana" /></div>
                </div>
                <div className="flex gap-3 mt-8">
                  ...
                  <button type="submit" disabled={isProcessing} className="flex-1 px-6 py-3 rounded-lg font-bold text-white transition-all disabled:opacity-50 hover:opacity-90" style={{ backgroundColor: '#132552' }}>
                    {isProcessing ? 'Processing...' : 'Pay USD ' + MEMBER_PRICE}
                  </button>
                </div>
              </form>
            )}

            {/* ── STEP 3B: Non-member form ── */}
            {applyStep === 'nonmember' && (
              <form onSubmit={handleNonMemberSubmit} className="p-8">
                <div className="p-4 rounded-xl mb-8 flex items-center justify-between" style={{ backgroundColor: '#FFF7ED', border: '1px solid #FED7AA' }}>
                  <div><p className="text-sm font-bold" style={{ color: '#92400E' }}>Standard Rate</p><p className="text-xs" style={{ color: '#4B5563' }}>You will automatically become a GoGMI member</p></div>
                ...
                <div className="space-y-4">
                  ...
                  <div><label className={labelClass} style={{ color: '#132552' }}>Position/Title <span className="text-red-500">*</span></label><input type="text" name="position" value={nonMemberForm.position} onChange={handleNonMemberChange} required className={inputClass} style={{ borderColor: '#E5E7EB' }} placeholder="Marine Safety Officer" /></div>
                  <div><label className={labelClass} style={{ color: '#132552' }}>Institution/Organisation <span className="text-slate-400 text-xs font-normal">(Optional)</span></label><input type="text" name="institution" value={nonMemberForm.institution} onChange={handleNonMemberChange} className={inputClass} style={{ borderColor: '#E5E7EB' }} placeholder="e.g. Ghana Maritime Authority" /></div>
                ...
                <div className="flex gap-3 mt-8">
                  ...
                  <button type="submit" disabled={isProcessing} className="flex-1 px-6 py-3 rounded-lg font-bold text-white transition-all disabled:opacity-50 hover:opacity-90" style={{ backgroundColor: '#16A34A' }}>
                    {isProcessing ? 'Processing...' : 'Pay USD ' + NON_MEMBER_PRICE}
                  </button>
                </div>
              </form>
            )}
      ...
      <section className="relative text-white py-24 md:py-32 overflow-hidden">
        ...
        <div className="container mx-auto max-w-6xl px-6 relative z-10">
          <div className="max-w-4xl">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl leading-tight mb-6" style={{ fontWeight: 900, letterSpacing: '-0.02em' }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={openApply} className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-bold transition-all hover:scale-105 shadow-2xl" style={{ backgroundColor: '#132552', color: 'white' }}>
                <span>Apply Now</span><ArrowRight className="w-5 h-5" />
              </button>
              <button onClick={handleBrochureDownload} className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-bold transition-all hover:scale-105 border-2 border-white/30 hover:bg-white/10">
        ...
        <div className="container mx-auto max-w-6xl px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center mb-16">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-black mb-6" style={{ color: '#132552', letterSpacing: '-0.01em' }}>Building Capacity for Maritime Safety</h2>
              <p className="text-lg leading-relaxed mb-6" style={{ color: '#4B5563' }}>Maritime and Inland Waterways (IWW) transportation safety is central to Africa's transport systems, supporting trade, passenger mobility, fishing, and livelihoods in coastal and riverine communities.</p>
              <p className="text-base leading-relaxed mb-6" style={{ color: '#6B7280' }}>However, preventable accidents on major IWW — such as the Volta Lake, Niger River, and Senegal River — continue to cause high fatalities, disrupt economies, and erode public confidence. This executive training course builds the capacity of Maritime Administrations and related agencies to efficiently conduct investigations of marine casualties and incidents, in accordance with SOLAS regulation XI-1/6 and the IMO Casualty Investigation Code.</p>
            </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex items-start gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-lg leading-relaxed mb-4" style={{ color: '#4B5563' }}>Between 2020 and 2025, numerous serious incidents occurred across African waterways: multiple capsizings on Volta Lake in Ghana, Niger River tragedies in Nigeria, and similar patterns in other states and waterways, with hundreds of deaths.</p>
                <p className="text-base leading-relaxed mb-4" style={{ color: '#6B7280' }}>Common causes include overloading, poor vessel maintenance, incompetent or unlicensed operators, absence of life-saving appliances, bad weather, weak enforcement of regulations, inadequate navigational aids, and human factors such as fatigue and drunkenness.</p>
                <p className="text-base leading-relaxed" style={{ color: '#6B7280' }}>These incidents highlight critical gaps: inadequate compliance monitoring, poor vessel maintenance, lack of standardized investigations to produce data for trend analysis and accident prevention.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto max-w-6xl px-6">
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-black mb-4" style={{ color: '#132552', letterSpacing: '-0.01em' }}>6 Comprehensive Modules</h2>
          </div>
          <div className="space-y-4">
                ...
                <button onClick={() => setActiveModule(activeModule === idx ? null : idx)} className="w-full p-6 flex items-center justify-between text-left hover:bg-gray-50 transition-all">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2"><span className="text-sm font-bold px-3 py-1 rounded-full" style={{ backgroundColor: `${module.color}15`, color: module.color }}>Module {module.number}</span></div>
                  ...
                  <div className={`transform transition-transform ${activeModule === idx ? 'rotate-180' : ''}`}>
                    <svg className="w-6 h-6" style={{ color: '#6B7280' }} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                  </div>
                </button>
                {activeModule === idx && (
      ...
      <section className="py-20" style={{ backgroundColor: '#F5F7FA' }}>
        <div className="container mx-auto max-w-6xl px-6">
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-black mb-4" style={{ color: '#132552', letterSpacing: '-0.01em' }}>Target Participants</h2>
          </div>
            ...
            {targetParticipants.map((p, i) => (
              <div key={i} className="bg-white rounded-xl p-6 shadow-sm hover:shadow-lg transition-all">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(142,52,0,0.1)' }}><Users className="w-5 h-5" style={{ color: '#8E3400' }} /></div>
                  <p className="text-base font-semibold" style={{ color: '#132552' }}>{p}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto max-w-6xl px-6">
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-black mb-4" style={{ color: '#132552', letterSpacing: '-0.01em' }}>Expected Outcomes</h2>
          </div>
          <div className="max-w-4xl mx-auto space-y-4">
            {expectedOutcomes.map((o, i) => (
              <div key={i} className="bg-gradient-to-r from-orange-50 to-white rounded-2xl p-6 border-l-4" style={{ borderColor: '#8E3400' }}>
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#8E3400', color: 'white' }}><span className="text-lg font-bold">{i+1}</span></div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {["Interactive lectures and regulatory framework analysis","Group case analysis based on real incidents","Practical use of investigation toolkits (timeline mapping, evidence logging)","Roleplay exercises (Maritime Administration, Wreck Commissioner, Assessors)","Report-writing and peer review sessions","Field scenario (Volta Lake case-based simulation)","Technology integration: drones for aerial photography, GPS tools for location logging"].map((m, i) => (
            ...
            <div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-lg mb-6">
                ...
                <ul className="space-y-3">
                  <li className="flex items-start gap-3"><div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(142,52,0,0.1)' }}><FileText className="w-4 h-4" style={{ color: '#8E3400' }} /></div><span className="text-sm leading-relaxed" style={{ color: '#4B5563' }}>IMO Code of International Standards and Recommended Practices for a Safety Investigation into a Marine Casualty or Marine Incident (MSC.255(84))</span></li>
              ...
              <div className="bg-gradient-to-br from-orange-900 to-orange-800 rounded-2xl p-6 text-white">
                ...
                <ul className="space-y-2 text-sm">
                  {["Strengthen regional institutional capacity for maritime safety governance","Support efforts to ensure safe, efficient, and sustainable inland water transport","Build a competent pool of regional investigators for marine accidents","Foster unified inter-agency response framework for casualty prevention"].map((t, i) => (<li key={i} className="flex items-start gap-2"><span>•</span><span>{t}</span></li>))}
                </ul>
              </div>
            </div>
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto max-w-4xl px-6 text-center">
          <h2 className="text-4xl font-black mb-4" style={{ color: '#132552' }}>Ready to Enroll?</h2>
          <p className="text-lg mb-8" style={{ color: '#6B7280' }}>Secure your place in the Marine Casualty Investigation & Safety Management Course.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={openApply} className="inline-flex items-center justify-center gap-2 px-10 py-4 rounded-xl font-bold transition-all hover:scale-105 shadow-xl" style={{ backgroundColor: '#8E3400', color: 'white' }}>
              <span>Apply Now</span><ArrowRight className="w-5 h-5" />
            </button>
            <button onClick={handleBrochureDownload} className="inline-flex items-center justify-center gap-2 px-10 py-4 rounded-xl font-bold transition-all hover:scale-105 border-2" style={{ borderColor: '#132552', color: '#132552' }}>
              <Download className="w-5 h-5" /><span>Download Brochure</span>
            </button>
          </div>
        </div>
