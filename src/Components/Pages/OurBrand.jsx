...
const BRAND_COLOR_HEXES = ['#132552', '#8E3400', '#F5F7FA', '#1F2933'];

const ColorSwatch = ({ color }) => {
  const { t } = useTranslation('ourBrand');
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        <p className="text-sm font-mono text-gray-500 mt-1">{copied ? t('colors.copied') : color.hex}</p>
...
const OurBrand = () => {
  const { t } = useTranslation('ourBrand');
  const BRAND_COLORS = t('colors.items', { returnObjects: true }).map((item, idx) => ({
    ...item,
    hex: BRAND_COLOR_HEXES[idx],
  }));
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-20 space-y-16">
        {/* Logo */}
        <section>
          <h2 className="text-2xl font-bold mb-6" style={{ color: '#132552' }}>{t('logo.heading')}</h2>
          <div className="grid sm:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 flex items-center justify-center">
              <img src="/GoGMI_PNG.png" alt={t('logo.altLight')} className="h-24 w-24 object-contain" />
            </div>
            <div className="rounded-2xl shadow-sm p-10 flex items-center justify-center" style={{ backgroundColor: '#132552' }}>
              <img src="/GoGMI_PNG.png" alt={t('logo.altDark')} className="h-24 w-24 object-contain" />
            </div>
          </div>
          <button
          ...
          >
            ...
            {t('logo.downloadButton')}
          </button>
        </section>

        {/* Colors */}
        <section>
          <h2 className="text-2xl font-bold mb-2" style={{ color: '#132552' }}>{t('colors.heading')}</h2>
          <p className="text-gray-600 mb-6">{t('colors.subtitle')}</p>
        ...
        {/* Typography */}
        <section>
          <h2 className="text-2xl font-bold mb-6" style={{ color: '#132552' }}>{t('typography.heading')}</h2>
        ...
        <section>
          ...
          >
            <h2 className="text-2xl font-bold text-white mb-2">{t('brandKit.heading')}</h2>
            ...
            >
              ...
              <span>{t('brandKit.downloadButton')}</span>
            </button>
          </div>
        </section>
      </div>
